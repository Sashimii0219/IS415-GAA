...
By plotting childcare data, we can see that
```{r}
tmap_mode('view')
tm_shape(childcare) +
  tm_dots(alpha=0.4, 
...
To overcome this problem there are 3 ways:
1. Delete duplicates, but some useful point events will be lost.
2. Jittering, which will add a small perturbation to the duplicate points so that they do not occupy the exact same space.
3. Make each point "Unique" and then attach the duplicates of the points to the patterns as marks, as attributes of the points. By doing this, we will need analytical technique that takes into account these marks.
...
We will be using density() of spatstat to calculate the kernel density estimation of childcare services in Singapore, with the following parameters:
