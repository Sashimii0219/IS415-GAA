...
## Importing Data
### Dataset
The two datasets that we are using today:
...
In this section, we will use poly2nb() of spdep package to compute contiguity weight matrices for the study area. This function:
- builds a neighbours list based on regions with contiguous boundaries.
- has a “queen” argument that takes TRUE or FALSE as options, with default as TRUE.
...
summary(wm_r)
```
Difference between Rook and Queen:
...
The first step is to determine the upper limit for distance band, which can be done by the following steps:
...
Red line = 1st nearest neighbours 
Black lines = links of neighbours within the cut-off distance of 62km.
